...
async fn replay_attack_is_rejected() {
    ...
    let params = build_params("req-replay", 9, &event);
    submit_signing_event(&ctx, params).await.expect("first submit");

    let params = build_params("req-replay-dup", 10, &event);
    let err = submit_signing_event(&ctx, params).await.expect_err("replay should error");

    match err {
        ThresholdError::EventReplayed(_) => {}
        other => panic!("expected EventReplayed, got {other:?}"),
    }
}
