...
use igra_core::foundation::{derivation_path_from_index, PeerId};
use igra_core::infrastructure::config::{PsktBuildConfig, PsktOutput, ServiceConfig};
...
async fn v1_service_signing_event_builds_pskt() {
    ...
    let derivation_index = env("KASPA_SIGNING_DERIVATION_INDEX").and_then(|v| v.parse().ok()).unwrap_or(0);
    ...
    let source_addresses =
        source_addresses.split(',').filter(|s| !s.trim().is_empty()).map(|s| s.trim().to_string()).collect::<Vec<_>>();
    ...
    let endpoint = iroh::Endpoint::builder().discovery(discovery).relay_mode(RelayMode::Disabled).bind().await.expect("endpoint");
    let gossip = iroh_gossip::net::Gossip::builder().spawn(endpoint.clone());
    let _router = iroh::protocol::Router::builder(endpoint).accept(iroh_gossip::net::GOSSIP_ALPN, gossip.clone()).spawn();
    ...
    let iroh_config = IrohConfig { network_id: 0, group_id: [9u8; 32], bootstrap_nodes: Vec::new() };

    let flow = Arc::new(
        ServiceFlow::new_with_iroh(&config, storage.clone(), gossip, signer, verifier, iroh_config).await.expect("service flow"),
    );
