...
async fn mock_node_tracks_utxos_and_submissions() {
    ...
    let address = Address::try_from("kaspadev:qzjwhmuwx4fmmxleyykgcekr2m2tamseskqvl859mss2jvz7tk46j2qyvpukx").expect("address");
    ...
    node.add_utxo(igra_core::infrastructure::rpc::UtxoWithOutpoint { address: Some(address.clone()), outpoint, entry });

    let utxos = node.get_utxos_by_addresses(std::slice::from_ref(&address)).await.expect("utxos");
    assert_eq!(utxos.len(), 1);

    let input = TransactionInput { previous_outpoint: outpoint, signature_script: Vec::new(), sequence: 0, sig_op_count: 0 };
    let output = TransactionOutput { value: 9_000, script_public_key: ScriptPublicKey::from_vec(0, vec![1, 2, 3]) };
