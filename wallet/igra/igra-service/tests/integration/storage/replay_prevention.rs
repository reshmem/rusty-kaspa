...
async fn replay_prevention_marks_seen_messages() {
    ...
    let first = storage
        .mark_seen_message(&PeerId::from("peer-1"), &session_id, 1, 100)
        .expect("mark seen");
    assert!(first, "first message should be new");

    let second = storage
        .mark_seen_message(&PeerId::from("peer-1"), &session_id, 1, 101)
        .expect("mark seen");
    assert!(!second, "duplicate message should be rejected");

    let third = storage
        .mark_seen_message(&PeerId::from("peer-1"), &session_id, 2, 102)
        .expect("mark seen");
