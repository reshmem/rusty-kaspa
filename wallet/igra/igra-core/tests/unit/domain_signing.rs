use igra_core::domain::pskt::multisig::{build_pskt, combine_pskts, extract_tx, finalize_multisig, sign_pskt, MultisigInput, MultisigOutput};
...
fn test_threshold_signing_when_all_sigs_present_then_finalizes_and_extracts_tx() {
    ...
    let signer1 = sign_pskt(pskt.clone().signer(), &kp1).expect("signer1");
    let signer2 = sign_pskt(pskt.clone().signer(), &kp2).expect("signer2");
