use crate::infrastructure::config::encryption::{encrypt_mnemonics, load_wallet_secret};
use crate::infrastructure::config::types::{
    ...
    AppConfig {
        service: ServiceConfig {
            ...
            pskt: PsktBuildConfig {
                ...
                outputs: Vec::new(),
                fee_payment_mode: crate::domain::FeePaymentMode::RecipientPays,
                fee_sompi: None,
                change_address: None,
            },
            hd: None,
        ...
        runtime: RuntimeConfig {
            ...
            session_expiry_seconds: Some(DEFAULT_SESSION_EXPIRY_SECS),
        },
        rpc: RpcConfig {
...
fn apply_runtime_section(config: &mut AppConfig, ini: &IniView<'_>) {
    ...
    if let Some(value) = ini_value(ini, "runtime", "session_expiry_seconds") {
        config.runtime.session_expiry_seconds = value.trim().parse::<u64>().ok();
    }
}
...
fn apply_rpc_section(config: &mut AppConfig, ini: &IniView<'_>) {
    ...
    if let Some(value) = ini_value(ini, "rpc", "rate_limit_rps") {
        config.rpc.rate_limit_rps = value.trim().parse::<u32>().ok();
    }
    if let Some(value) = ini_value(ini, "rpc", "rate_limit_burst") {
        config.rpc.rate_limit_burst = value.trim().parse::<u32>().ok();
    }
}
...
fn apply_group_section(config: &mut AppConfig, ini: &IniView<'_>) -> Result<(), ThresholdError> {
    ...
    let network_id = match ini_value(ini, "group", "network_id") {
        Some(v) => v
            .trim()
            .parse::<u8>()
            .map_err(|_| ThresholdError::ConfigError(format!("invalid group.network_id: '{v}'")))?,
...
fn apply_hyperlane_section(config: &mut AppConfig, ini: &IniView<'_>) -> Result<(), ThresholdError> {
    ...
    if let Some(value) = ini_value(ini, "hyperlane", "default_derivation_path") {
        config.hyperlane.default_derivation_path = non_empty(&value);
    }
...
fn parse_fee_payment_mode(value: &str) -> Result<crate::domain::FeePaymentMode, ThresholdError> {
    ...
    if let Some(rest) = trimmed.strip_prefix("split:") {
        ...
        if tokens.len() == 2 {
            ...
            return Ok(crate::domain::FeePaymentMode::Split { recipient_parts, signer_parts });
        }
        return Err(ThresholdError::Message(
            "invalid pskt.fee_payment_mode split: expected split:<recipient_parts>:<signer_parts>".to_string(),
        ));
    }
    match trimmed.as_str() {
        "recipient_pays" => Ok(crate::domain::FeePaymentMode::RecipientPays),
        "signers_pay" => Ok(crate::domain::FeePaymentMode::SignersPay),
