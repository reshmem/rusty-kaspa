...
pub fn current_timestamp_nanos_env(env_var: Option<&str>) -> Result<u64, ThresholdError> {
    if let Some(var) = env_var {
        if let Ok(value) = std::env::var(var) {
            return value.parse::<u64>().map_err(|err| ThresholdError::EncodingError(err.to_string()));
        }
    }
    let now = SystemTime::now().duration_since(UNIX_EPOCH).map_err(|err| ThresholdError::EncodingError(err.to_string()))?;
