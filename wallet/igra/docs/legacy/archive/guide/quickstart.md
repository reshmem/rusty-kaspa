...
- Mnemonics (generated per signer; never shared)
- Iroh Ed25519 seeds
- `secrets.bin` passphrases (never shared)
...
```bash
cd rusty-kaspa/wallet/igra
cargo run --release --bin devnet-keygen -- --num-signers 1 --signer-profile signer-01 > signer-01-keys.json
```
  ...
  "signers": [
    {
      ...
      "pubkey_hex": "03a4ebef8e3553bd43ea837e07cfe2cfee87f1827799e5e00ef134eb9eb942c43c",
      "address": "kaspadev:..."
    }
  ]
}
...
signer-01: a4ebef8e3553bd43ea837e07cfe2cfee87f1827799e5e00ef134eb9eb942c43c
signer-02: b93ff1e12a7c4f3d5e6b9a8c7d2e1f0a3b5c4d6e8f9a0b1c2d3e4f5a6b7c8d9e
signer-03: ca1582f4d6e9b7a3c5f2e8d1b4a7c9e6f3d0b5a8c2e4f1d7b9a6c3e0f5d8b1a4
...
1. `a4ebef8e...` (signer-01)
2. `b93ff1e1...` (signer-02)
3. `ca1582f4...` (signer-03)
...
```bash
# Start the node
KASPA_DATA_DIR=.igra/signer-01 kaspa-threshold-service --config igra-config.toml --profile signer-01
...
# Per-signer overrides (in [profiles.signer-01.iroh])
...
# change_address defaults to the derived multisig source address.

[service.hd]
# Devnet/testnet only (mainnet forbids mnemonic-based signing).
key_type = "hd_mnemonic"
required_sigs = 2
derivation_path = "m/45'/111111'/0'/0/0"  # Optional, default is root
# xpubs is optional when `service.pskt.redeem_script_hex` is set.
...
# NOTE: signing keys are loaded from `${data_dir}/secrets.bin` using the active profile (`--profile signer-01`).
...
# NOTE: signing keys are loaded from `${data_dir}/secrets.bin` using the active profile (`--profile signer-02`).
...
# NOTE: signing keys are loaded from `${data_dir}/secrets.bin` using the active profile (`--profile signer-03`).
...
### 11.1 Create Per-Signer `secrets.bin`

**Each signer (in their terminal, privately)**:

```bash
# Preferred (devnet): generate one secrets.bin per signer directory.
# This writes:
#   .igra/signer-01/secrets.bin
#   .igra/signer-02/secrets.bin
#   .igra/signer-03/secrets.bin
devnet-keygen --format file-per-signer --output-dir .igra --passphrase "devnet-secret" --overwrite
```

**Security notes**:
- One `secrets.bin` per signer (no sharing).
- In real deployments, use different passphrases per signer and store them in a secret manager.
...
[INFO] Profile: signer-01
[INFO] Data directory: .igra/signer-01
...
grep "Discovered peers" .igra/signer-01/igra.log

# Expected: All N-1 other peers discovered
# Signer 01 should see: [signer-02, signer-03]
# Signer 02 should see: [signer-01, signer-03]
# Signer 03 should see: [signer-01, signer-02]
```

**Discord post** (each signer):

```markdown
**Signer 01 (Alice)**: ✅ Gossip connected. Discovered 2 peers.
**Signer 02 (Bob)**: ✅ Gossip connected. Discovered 2 peers.
**Signer 03 (Charlie)**: ✅ Gossip connected. Discovered 2 peers.
...
```bash
tail -f .igra/signer-01/igra.log | grep -E "proposal|commit|sign|submit"
```
...
|----------|---------|---------|
| `IGRA_SECRETS_PASSPHRASE` | Decrypt `secrets.bin` | `"my-secure-passphrase"` |
| `KASPA_DATA_DIR` | Data directory | `.igra/signer-01` |
...
**Causes**:
- Missing/invalid signer secret material (mnemonic/payment_secret in devnet/testnet, or raw private key in mainnet)
- Wrong derivation path
- Wrong `IGRA_SECRETS_PASSPHRASE` (cannot decrypt `secrets.bin`)
- Wrong `--profile` / wrong secrets file (profile-to-secret mismatch)

**Fix**:
- Verify the signer’s secret material corresponds to the expected pubkey (test derivation offline)
- Check `derivation_path` in config (default: `m/45'/111111'/0'/0/0`)
- Confirm `IGRA_SECRETS_PASSPHRASE` is correct
- Verify mnemonic in `secrets.bin` (`secrets-admin --path <...>/secrets.bin verify-mnemonic --profile signer-XX`)
...
```bash
# Check stored events
sqlite3 .igra/signer-01/rocksdb/events.db "SELECT * FROM events;"
